...
@predios.route('/predioss/v1', methods=['GET'])
def getMensaje():
    result={}
    result["data"]='flask-crud-backend'
    return jsonify(result)

@predios.route('/predioss/v1/list', methods=['GET'])
def getPredios():
    result={}
...
@predios.route('/predioss/v1/insert', methods=['POST'])
def insert():
    result={}
...
@predios.route('/predioss/v1/update', methods=['POST'])
def update():
    result={}
    body=request.get_json()
...
@predios.route('/predioss/v1/delete', methods=['POST'])
def delete():
    result={}
    body=request.get_json()
    id_predio=body.get('id_predio')
